...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 50 == 0:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(model_save_path, f"epoch{epoch}.pth"),
            )
            if ema is not None:
                ema.restore()
...
dataset2 = Basedataset(
    "data/STS24_2D_XRAY/Resized-Train-Unlabeled",
    "data/STS24_2D_XRAY/Resized-Train-Unlabeled-Masks-v2",
)

dataset = ConcatDataset([dataset1, dataset2])

train_dataset = AugDataset(dataset, transform=transform)
